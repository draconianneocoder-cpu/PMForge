...
- **`audit_events`** — tamper-evident compliance audit foundation: `project_id`, `sequence_number`, previous/current SHA-256 hashes, event/entity metadata, canonical before/after JSON, actor/session metadata, timestamp, and optional signature fields. Project create/update/delete, chart create/update/delete, document create/update/delete, baseline create/delete, and document signature outcomes now write hash-chained events. Project Settings can export private JSON audit verification and repair-evidence artifacts. Remaining audit slices: approval checkpoints and combined-report signature checkpoints after a stable report entity ID exists.
