...
- **CI/CD moved from GitLab to GitHub Actions.** `.github/workflows/ci.yml` (verify / build / lint / security) + `release.yml` (native multi-OS Wails builds → GitHub Release) replace `.gitlab-ci.yml`. Two clean-checkout gotchas are baked in: (a) the root package's `//go:embed all:frontend/dist` needs the frontend built before any Go compile, so CI builds it (or stubs the dir for vet/lint); (b) `wails@v2.9.2` transitively pulls `golang.org/x/tools@v0.17.0`, which won't compile under Go 1.26, so the Wails CLI is installed with `GOTOOLCHAIN=go1.24.0` while the app still builds under the repo's 1.26.4. `make fonts` is no longer a hard gate (flaky CDNs; the app degrades to core fonts).
