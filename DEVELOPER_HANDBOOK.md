...
- **CI/CD moved from GitLab to GitHub Actions.** `.github/workflows/ci.yml` (verify / build / lint / security) + `release.yml` (native multi-OS Wails builds → GitHub Release) replace `.gitlab-ci.yml`. Two clean-checkout gotchas are baked in: (a) the root package's `//go:embed all:frontend/dist` needs the frontend built before any Go compile, so CI builds it (or stubs the dir for vet/lint); (b) Wails was bumped 2.9.2 → **2.12.0** (2026-06-22), which raises `golang.org/x/tools` to v0.30.0 — it compiles cleanly under Go 1.26, so the earlier `GOTOOLCHAIN=go1.24.0` workaround for the CLI install was removed; the app builds under the repo's Go 1.26.5. `make fonts` is no longer a hard gate (flaky CDNs; the app degrades to core fonts). golangci-lint is pinned to v2.12.2 (action `golangci/golangci-lint-action@v9`); all workflow actions use node24-era major versions (checkout@v7, setup-go@v6, setup-node@v6, upload-artifact@v7, download-artifact@v8).
