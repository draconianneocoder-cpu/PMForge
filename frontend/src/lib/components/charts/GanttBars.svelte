...
{:else}
  <svg width={canvasW} height={canvasH} role="img" aria-label={ariaLabel}>
    <!-- day grid -->
    ...
    {#each layout.rows as r, i (r.id)}
      ...
      <g>
      {#if barTitle}<title>{barTitle(r)}</title>{/if}
      <!-- baseline ghost -->
      {#if bb}
